...
img1 <- readPNG("schematics/Consort-01.png")
g_consort <- rasterGrob(img1, interpolate=TRUE)

# --------------- #
# Study flowchart #
# --------------- #

img2 <- readPNG("schematics/analysis schema-01.png")
g_flowchart <- rasterGrob(img2, interpolate=F)
...
p_cfmedip_vs_natera_correlation <- pd_glmprediction_by_timepoint %>%
  ...
  labs(
    x = 'Joint cancer log-odds from cfMeDIP-seq',
...
val_total_significant_variable_length <- pd_fragment_ratio_var_regions %>%
  ...
  .$width %>%
  sum()
val_total_significant_variable_length_mb <- value_to_string(val_total_significant_variable_length / 1e6)
...
val_percent_discordant <- value_to_percent(val_cmc_csm_discordant / nrow(plotdata_cfmedip_response))

val_cmc_csm_low_threshold <- 1

val_cmc_csm_discordant_low <- plotdata_cfmedip_response %>%
  filter(change != change_group_natera) %>%
  filter(SB < val_cmc_csm_low_threshold | natera_SB < val_cmc_csm_low_threshold) %>%
  nrow()
...
val_uva_fls_pfs_p <- pval_to_string(pfit_fls_survplot_pfs$cox$p, n_digits = 4)
